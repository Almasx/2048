...
import { themeAtom } from "./ThemeSwitch";
...
export function Grid({ board }: { board: Board }) {
  ...
  return (
    <div
      className="grid gap-3 rounded-2xl border border-gray-light p-4 dark:border-gray-dark-secondary sm:gap-4"
      style={{
        boxShadow:
      ...
      {board.map((row, rowIndex) =>
        row.map((value, colIndex) => (
          <div
            ...
            className="h-16 w-16 overflow-clip rounded-3xl border border-gray-light bg-[#f3f3f3] 
                       duration-150 hover:border-gray-light-secondary hover:bg-[#f4f4f4] dark:border-gray-dark-secondary dark:bg-[#060606] dark:hover:border-gray-dark 
                      dark:hover:bg-[#0f0f0f] sm:h-24 sm:w-24"
          >
            <Tile value={value} />
          </div>
        ))
  ...
  return (
    <div className="relative h-full w-full">
      {value !== 0 && (
        <div
          ...
          style={{
            ...
            width: bgSize,
            height: bgSize,
            boxShadow: `0px 0px 20px ${theme ? "" : "15px"} #1E2BEC`,
          }}
        />
      )}
      <div
      ...
      >
        <div className="">
          {value !== 0 && (
            <p className="text-center text-5xl font-bold text-white">{value}</p>
          )}
        </div>
      </div>
    </div>
